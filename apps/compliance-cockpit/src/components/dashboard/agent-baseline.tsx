...
export function AgentBaseline({ agentId }: Props) {
  ...
  if (!data || data.total === 0) {
    return (
      <div className="rounded-lg p-4 text-xs text-center" style={{ background: 'hsl(var(--secondary))', color: MUTED, border: `1px solid ${BORDER}` }}>
  ...
  return (
    <div className="rounded-xl border p-4 space-y-4" style={{ borderColor: BORDER, background: 'hsl(var(--card))' }}>
      <div className="flex items-center justify-between">
        ...
        <span className="text-[10px] font-mono px-2 py-0.5 rounded" style={{ background: 'hsl(var(--secondary))', color: MUTED }}>{agentId}</span>
      </div>
      ...
      <div className="grid grid-cols-4 gap-2">
        {[
          ...
          <div key={label} className="rounded-lg p-2.5 text-center" style={{ background: 'hsl(var(--secondary))' }}>
          ...
          {(['LOW', 'MEDIUM', 'HIGH', 'CRITICAL'] as const).map(level => {
            ...
            return (
              <div key={level} className="flex items-center gap-1">
                ...
                <div className="rounded-full" style={{ width: '60px', height: '5px', background: 'hsl(var(--secondary))' }}>
          ...
          <div className="space-y-1">
            {(data.top_tools as any[]).slice(0, 5).map((t: any) => (
              <div key={t.tool_name} className="flex items-center gap-2">
                ...
                <div className="flex-1 rounded-full" style={{ height: '5px', background: 'hsl(var(--secondary))' }}>
